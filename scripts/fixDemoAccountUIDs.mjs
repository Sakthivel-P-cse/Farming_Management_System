// Fix demo account Firestore documents to use Firebase UID as document ID
// Usage: node scripts/fixDemoAccountUIDs.mjs
...
const demoAccountEmails = [
  'district2@example.com',
  'district3@example.com',
  'district4@example.com',
  'district5@example.com',
  'demo.district@glytch.local',
];

async function fixDemoAccountUIDs() {
  try {
    console.log('🔧 Fixing demo account Firestore documents...\n');
    ...
    for (const userDoc of snapshot.docs) {
      ...
      // Check if this is a demo account with a custom ID
      if (userData.role === 'district' && demoAccountEmails.includes(userData.email)) {
        const email = userData.email;
        
        // Check if document ID is already a Firebase UID (long alphanumeric string)
        if (documentId.length > 20 && !documentId.startsWith('district')) {
          console.log(`✓ ${email}`);
          ...
          if (userData.uid) {
            ...
            await setDoc(doc(db, 'users', userData.uid), {
              ...
              name: userData.name,
              role: userData.role,
              district: userData.district || 'Demo/Test District',
              contact: userData.contact || '9000000000',
              villagesManaged: userData.villagesManaged || 5,
